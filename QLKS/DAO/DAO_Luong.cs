...
namespace DAO
{
    public class DAO_Luong
    {
        ...
        public void LoadDGVForm(TextBox maLuong, ComboBox maNV, TextBox thang, TextBox nam, TextBox soNgayLamViec, TextBox soGioTangCa, TextBox luongCoBan, TextBox phuCap, TextBox thuong, TextBox khauTru, TextBox tongLuong, DateTimePicker ngayTinhLuong, DataGridView data)
        {
            using (DBQuanLyKhachSanDataContext db = new DBQuanLyKhachSanDataContext(ThayDoiChuoi.GetConnectionString()))
            {
                if (data.SelectedCells.Count > 0)
                {
                    ...
                    soNgayLamViec.Text = row.Cells[4].Value.ToString().Trim();
                    soGioTangCa.Text = row.Cells[5].Value.ToString().Trim();
                    luongCoBan.Text = row.Cells[6].Value.ToString().Trim();
                    phuCap.Text = row.Cells[7].Value.ToString().Trim();
                    thuong.Text = row.Cells[8].Value.ToString().Trim();
                    khauTru.Text = row.Cells[9].Value.ToString().Trim();
                    tongLuong.Text = row.Cells[10].Value.ToString().Trim();
                    ngayTinhLuong.Text = row.Cells[11].Value.ToString().Trim();
